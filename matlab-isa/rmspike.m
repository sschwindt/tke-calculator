...
function dspout = rmspike(dataset, f, lambda_a, k)
% dataset - the matrix Data in the main program
% f - sampling frequency
% lambda_a - multiplier of gravitational acceleration (acceleration threshold)
% k - multiplier of velocity stdev (velocity threshold)
% Goring & Nikora (2002) suggest lambda_a = 1.0 ~ 1.5 and k = 1.5, but we shall use lambda_a = 1.0 and k = 3 ~ 9.
% Also, SonTek, Nortek, and Lei recommend the SNR and correlation thresholds to be 15 and 70 respectively.
% Almost all the data points have high SNR, but the correlation can be low.

global name; % The variable "name" is still used within this function rmspike().
global hdr_stat;
global side;
global pos;

out = [dataset(:,1), dataset(:,4:7)]; % Time series. In column order: t (s), u (m/s), v (m/s), w1 (m/s), w2 (m/s).
...
a_x = cat(1, NaN, out(2:row, 2) - out(1:(row-1), 2)) * f; % x acceleration (m/s^2)
a_y = cat(1, NaN, out(2:row, 3) - out(1:(row-1), 3)) * f; % y acceleration (m/s^2)
a_z1 = cat(1, NaN, out(2:row, 4) - out(1:(row-1), 4)) * f; % z1 acceleration (m/s^2)
if side ~= 1
	a_z2 = cat(1, NaN, out(2:row, 5) - out(1:(row-1), 5)) * f; % z2 acceleration (m/s^2)
end
...
spikeu = 0; spikev = 0; spikew1 = 0; % Initialization of total counts
	for j = 2:length(u)-1 % Count
	  if (~((a_x(j) > -a_cr) && (a_x(j) < a_cr)) && ~isnan(a_x(j))) % NaN values are also included.
		  u(j) =  NaN;
		  spikeu = spikeu + 1;
	  end
